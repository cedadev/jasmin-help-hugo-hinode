...
{{< accordion id="accordion-1" >}}
  ...
  {{< accordion-item title="Why am I asked for a password when logging in to mass-cli.jasmin.ac.uk?" >}}
    ...
    The first is if you do not have permission to access the machine. A quick method to check is to verify if you are a member of the `moose` user group. It should be listed when you use the ‘groups’ command whilst logged into JASMIN login node:
